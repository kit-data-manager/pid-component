...
// ---------------------------------------------------------------------------
// Helper component: renders children HTML and runs initPidDetection on mount
// ---------------------------------------------------------------------------
 ...
 * emails, etc.) and replaces them with interactive `<pid-component>`
 * elements.
 *
 * In React, wrap the target content in a ref-based container and call
 * `initPidDetection()` inside a `useEffect`. The returned controller
 * provides `stop()`, `rescan()`, and `destroy()` for lifecycle control.
 *
 * ```tsx
 * import { initPidDetection } from '@kit-data-manager/pid-component';
 *
