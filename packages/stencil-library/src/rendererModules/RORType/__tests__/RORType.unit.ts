...
describe('RORType', () => {
  ...
  describe('init()', () => {
    ...
    it('handles organization with no locations', async () => {
      (global.fetch as any).mockResolvedValue({
        ok: true,
      ...
      const countryItem = rt.items.find(i => i.keyTitle === 'Country');
      expect(countryItem).toBeUndefined();
    });

    it('handles organization with no links', async () => {
      (global.fetch as any).mockResolvedValue({
        ok: true,
    ...
    it('handles organization with no external_ids', async () => {
      (global.fetch as any).mockResolvedValue({
        ok: true,
      ...
      expect(gridItem).toBeUndefined();
    });

    it('handles organization with no relationships', async () => {
      (global.fetch as any).mockResolvedValue({
        ok: true,
      ...
      const relItem = rt.items.find(i => i.keyTitle === 'Predecessor');
      expect(relItem).toBeUndefined();
    });

    it('handles multiple names with different types', async () => {
      (global.fetch as any).mockResolvedValue({
        ok: true,
        json: vi.fn().mockResolvedValue({
          ...rorApiResponse,
          names: [
            { value: 'Full Name', types: ['ror_display'] },
            { value: 'SHORT', types: ['acronym'] },
            { value: 'alias1', types: ['alias'] },
            { value: 'alias2', types: ['alias'] },
            { value: 'label1', types: ['label'] },
          ],
        }),
      });
    ...
    it('handles deprecated organization status', async () => {
      (global.fetch as any).mockResolvedValue({
        ok: true,
      ...
      expect(statusItem.value).toBe('deprecated');
    });
