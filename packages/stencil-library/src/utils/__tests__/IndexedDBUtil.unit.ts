...
describe('Database', () => {
  ...
  describe('normalizeKey() (via public API)', () => {
    ...
    it('handles objects that cannot be stringified', async () => {
      ...
      const nonStringifiable = { circular: null };
      (nonStringifiable as any).circular = nonStringifiable;

      await db.getEntity(nonStringifiable as unknown as string, settings);
  ...
  describe('deleteEntity() with relations', () => {
    it('deletes matching relations from the index', async () => {
      const mockCursor = {
        value: { start: 'related-value', end: 'other-value' },
        primaryKey: 1,
      ...
      await db.deleteEntity('related-value');
