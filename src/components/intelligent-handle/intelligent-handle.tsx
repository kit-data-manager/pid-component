    ...
    render() {
        return (
            <Host class="inline-flex flex-grow w-fit max-w-screen-lg font-sans">
                <details
                    class={"rounded-md shadow-md bg-white border text-clip align-baseline inline-flex flex-grow max-w-screen-lg font-sans p-0.5 open:p-1 open:align-top"} open={this.openStatus}
                                ...
                                (this.pidRecord !== undefined) ? this.pidRecord.values.filter((value) => {
                                    ...
                                    return (
                                        <tr class={this.changingColors ? "odd:bg-slate-200" : ""}>
                                            ...
                                            <td class={"align-top text-clip overflow-x-scroll pl-2 p-1"}>
                                                {
                                                    PID.isPID(value.data.value) ?
                                                        (this.loadSubcomponents ?
                                                            ...
                                                            /> :
                                                            <a href={`https://kit-data-manager.github.io/fairdoscope/?pid=${value.data.value.toString()}`}
                                                               class={"underline"}
                    ...
                    </div>
                    {
                        (this.pidRecord !== undefined) ?
                            ...
                            <a href={`https://kit-data-manager.github.io/fairdoscope/?pid=${this.pidRecord.pid.toString()}`}
                               class={"p-1.5 bg-slate-200 font-semibold text-blue-500 text-sm rounded border mr-1"}>Open in FAIR-DOscope</a>
                            <a href={`https://hdl.handle.net/api/handles/${this.pidRecord.pid.toString()}`}
                               class={"p-1.5 bg-slate-200 font-semibold text-blue-500 text-sm rounded border ml-1"}>Resolve via Handle.net</a>
                        </span>
                            : ""
                    }
                </details>
