  ...
  render() {
    return <Host>
      <a href={`https://orcid.org/${this.orcid}`}
         class={"bg-gray-100 border shadow-md p-0.5 rounded-md inline-flex"}
         target={"_blank"}>
        <span class={"inline-flex flex-nowrap flex-row self-center"}>
          <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256"
               class={"h-6 p-0.5 self-stretch"}>
            <path class="st0"
        ...
        </span>
        {
          this.orcidInfo !== undefined
            ? <span
              class="flex-nowrap p-1 font-mono font-medium underline text-sm">{this.orcidInfo.familyName}, {this.orcidInfo.givenNames} ({
              this.orcidInfo.getAffiliationAt(this.affiliationAt) !== undefined
                ? `${this.orcidInfo.getAffiliationAt(this.affiliationAt).organization} [${
                  this.orcidInfo.getAffiliationAt(this.affiliationAt).department}], `
