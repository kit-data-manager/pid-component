...
1. Builds `stencil-library` first (`stencil build --docs`), generating the web components, the
   `dist/`, `hydrate/`, and `loader/` outputs, plus the auto-generated framework proxy code in the
...
to display framework-specific stories from the React and Vue wrapper packages alongside the main
Web Components stories. To run the full composed Storybook locally:
...
1. Start the React sub-Storybook on port 6007
2. Start the Vue sub-Storybook on port 6008
3. Wait for both, then start the main composed Storybook on port 6006

> Angular is excluded from composition because `@storybook/angular` requires a full Angular CLI
> application scaffold, which is incompatible with a standalone library project. The Angular
> wrapper is a thin proxy layer over the same web components shown in the main Storybook.
...
This builds the main Storybook and both framework sub-Storybooks into `storybook-static/`,
with the sub-Storybooks placed in subdirectories (`react-storybook/`, `vue-storybook/`).
...
**Unit and E2E tests (Stencil):**

```bash
npm test                      # runs tests in all packages via Lerna
```

Or from `packages/stencil-library` directly:
...
**Storybook tests (a11y, interaction, visual):**

```bash
npm run test-storybook        # runs from repo root
```

This uses the `@storybook/addon-vitest` plugin which renders every story in a headless Chromium
browser and runs any `play()` functions and accessibility audits defined in the stories.
...
Runs on every push and pull request. Tests against Node.js 22, 24, and 25:

1. `npm ci` -- install dependencies
2. `npx playwright install --with-deps chromium` -- install browser for E2E tests
3. `npx lerna run build` -- build all packages
4. `npx lerna run lint` -- ESLint
5. `npx lerna run format:check` -- Prettier
6. `npx lerna run test` -- Vitest spec and E2E tests
7. `npm run build-storybook` -- build static Storybook
8. `npm run test-storybook` -- Storybook Vitest addon tests
...
Runs on every push. Uploads the main Web Components Storybook to
...
The deployed Storybook includes the main Web Components stories plus the composed React and Vue
sub-Storybooks (accessible via the sidebar).

### npm Packages

The four npm packages are published independently (Lerna independent versioning). To publish:
...
preview for each PR. It runs automatically via `chromatic.yml` on every push.
